...
try:
    ...
    print(f"低库存（0 < 库存 <= 最低）：{low_stock}")
    
    # 正常的
    cursor.execute("""
        SELECT COUNT(*) FROM spare_part 
        WHERE current_stock > min_stock 
        AND (max_stock IS NULL OR current_stock < max_stock)
    """)
    normal_detailed = cursor.fetchone()[0]
    print(f"正常库存：{normal_detailed}")
    ...
    """)
    overstocked_detailed = cursor.fetchone()[0]
    print(f"超储库存：{overstocked_detailed}")
