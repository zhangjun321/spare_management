...
class Transaction(db.Model):
    ...
    transaction_number = db.Column(db.String(50), unique=True, nullable=False, index=True, comment='交易编号')
    ...
    batch_id = db.Column(db.Integer, db.ForeignKey('batch.id'), index=True, comment='批次 ID')
    ...
    transaction_date = db.Column(db.DateTime, default=datetime.now, index=True, comment='交易日期')
    remark = db.Column(db.Text, comment='备注')
    created_at = db.Column(db.DateTime, default=datetime.now, index=True, comment='创建时间')
    created_by = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False, index=True, comment='创建人 ID')
    confirmed_by = db.Column(db.Integer, db.ForeignKey('user.id'), index=True, comment='确认人 ID')
    confirmed_at = db.Column(db.DateTime, index=True, comment='确认时间')
    status = db.Column(db.String(20), default='pending', index=True, comment='状态')
